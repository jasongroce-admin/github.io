...
function initWallet(){setTokens(getTokens()); const el=document.getElementById('soundBtn'); if(el)el.textContent='Sound: '+(soundOn()?'On':'Off'); ensureArcadeDock(); ensureTopAtmButton();}
...
function ensureTopAtmButton(){const top=document.querySelector('.top');if(!top||document.getElementById('atmTopBtn'))return;const b=document.createElement('button');b.id='atmTopBtn';b.className='btn';b.type='button';b.textContent='ATM';b.onclick=openAtm;top.appendChild(b);}

function ensureArcadeDock(){if(document.getElementById('arcadeDock'))return;const d=document.createElement('div');d.id='arcadeDock';d.className='arcade-dock';d.innerHTML="<a class='btn' href='index.html'>Arcade Home</a><button class='btn' id='openAdvice' type='button'>Golden Advice</button><button class='btn' id='openAtmBtn' type='button'>ATM</button>";document.body.appendChild(d);const panel=document.createElement('div');panel.id='adviceDrawer';panel.className='advice-drawer';panel.innerHTML="<h4>Golden Advice (Quick)</h4><p>Open full advisor tools from the Arcade home page.</p><ul><li>Roulette: track color streaks, switch after long runs.</li><li>Blackjack: follow dealer up-card strategy.</li><li>Craps: two-dice trends matter for pace bets.</li></ul><a class='btn' href='index.html'>Open Full Golden Advice</a><button class='btn' id='closeAdvice' style='margin-left:8px'>Close</button>";document.body.appendChild(panel);document.getElementById('openAdvice').onclick=()=>panel.classList.add('open');document.getElementById('closeAdvice').onclick=()=>panel.classList.remove('open');document.getElementById('openAtmBtn').onclick=openAtm;}
