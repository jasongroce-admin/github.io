...
function setMode(mode) {
  ...
  overlayStory.textContent = mode === 'mission'
    ...
    : 'Free Roam practice: no timer pressure. Learn curves, hazard behavior, and boost timing.';
}
...
function nextDispatch() {
  ...
  game.callTargetDistance = game.callStartDistance + game.level * 150;
...
function spawn(type) {
  const laneX = ROAD_X + 34 + Math.random() * (ROAD_W - 68);
  ...
  if (type === 'ped') obj.x = ROAD_X - 8 + Math.random() * (ROAD_W + 16);
...
function completeCall() {
  ...
  nextDispatch();
  showHint(`Dispatch: Call complete. New target: ${game.targetAddress}`);
...
function update(dt) {
  ...
  if (keys.has(' ') || keys.has('Space')) player.boost = Math.min(4.5, player.boost + 7.8 * dt);
  ...
  player.x += steering * 255 * dt + wind;
  game.speed = Math.max(5.3, Math.min(20, game.speed + throttle * 3.5 * dt));
  ...
  const spawnRate = Math.max(0.28, 0.82 - game.level * 0.08);
  ...
  if (game.bonusClock > 4.2) {
    game.bonusClock = 0;
    spawn(Math.random() > 0.4 ? 'clock' : 'shield');
  }

  const flow = (game.speed + player.boost) * 10.5;
